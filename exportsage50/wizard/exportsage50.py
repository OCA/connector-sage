    ...
    def create_report(self, cr, uid, ids, context=None):
        ...
        if not data['invoice_ids']:
            raise osv.except_osv(
                _('Error'), _('You have to select at least 1 Invoice. And try again'))
        ...
        for line in line_obj.browse(cr, uid, data['invoice_ids'], context):
            ...
            if line.date_invoice:
                entry_date = datetime.strptime(
                    line.date_invoice, "%Y-%m-%d").strftime('%m-%d-%Y')  # date format : mm-dd-yyyy
            else:
                entry_date = ""
            ...
            list_id = []
            # Paid by source (20 Chars) : Blank- pay later and cash Cheque
            ...
            if line.payment_ids:
                ...
                lastId = max(list_id)
                # acceder à partir du dernier paiement à l'objet
                # account_move_line
            ...
            sale_invoice = ','.join(
                ['"%s"' % field_sale_invoice for field_sale_invoice in fields_sale_invoice])
            ...
            if product_ids:
                for product in account_invoice_line_obj.browse(cr, uid, product_ids):
                    ...
                    one_product_invoice = ','.join(
                        ['"%s"' % field_one_product_invoice for field_one_product_invoice in fields_one_product_invoice])
                    ...
                    if product.invoice_line_tax_id:
                        for one_taxe in product.invoice_line_tax_id:
                            # or one_taxe.description or one_taxe.name
                            tax_name = one_taxe.description
                            ...
                            tax_product_line = ',' + \
                                ','.join(
                                    ['"%s"' % field_tax_product_line for field_tax_product_line in fields_tax_product_line])
                    ...
                    product_line_invoice_with_taxe += one_product_invoice + \
                        tax_product_line + '\n'
        ...
        self.write(cr, uid, ids, {
                   'state': 'get', 'data': out, 'name': this.name, 'format': this.format}, context=context)
