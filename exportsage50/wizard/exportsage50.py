...
class exportsage(orm.Model):
    ...
    _columns = {
        ...
        'invoice_ids': fields.many2many(
            'account.invoice',
            'sale_order_invoice_export_rel',
            'order_id',
            'invoice_id',
            'Invoices',
            required=True,
            help="This is the list of invoices that have been generated "
                 "for this sales order. The same sales order may have been "
                 "invoiced in several times (by line for example)."),
    }
    ...
    def create_report(self, cr, uid, ids, context=None):
        ...
        if not data['invoice_ids']:
            raise orm.except_orm(
                _('Error'),
                _('You have to select at least 1 Invoice. And try again'))
        ...
        for line in line_obj.browse(
                cr, uid, data['invoice_ids'], context=context):
            ...
            # Customer line
            fields = [
                costumer_name, onetimefield, contact_name, street1, street2,
                city, province_state, zip_code, country, phone1, mobile, fax,
                email
            ]
            ...
            entry_date = datetime.strptime(
                line.date_invoice, "%Y-%m-%d").strftime('%m-%d-%Y')\
                if line.date_invoice else ""
            # Type of payment (between 0 and 3)
            # 0 = pay later , 1 = cash , 2 = cheque , 3 = credit card
            # Select last payment
            list_id = []
            # Paid by source (20 Chars) : Blank- pay later
            # and cash Cheque number or credit card
            paid_by_source = ""
            if line.payment_ids:
            ...
            freight_amount = "0.0"
            fields_sale_invoice = [
                no_of_details, order_no, invoice_no, entry_date, paid_by_type,
                paid_by_source, total_amount, freight_amount]
            sale_invoice = ','.join([
                '"%s"' % one_field for one_field in fields_sale_invoice
            ])
            output += sale_invoice.encode('UTF-8') + '\n'
            product_line_invoice_with_tax = ""
            # Sale invoice detail lines
            product_ids = account_invoice_line_obj.search(
                cr, uid, [('invoice_id', '=', line.id)], context=context)

            if product_ids:
                for product in account_invoice_line_obj.browse(
                        cr, uid, product_ids, context=context):
                    item_number = unicode(product.name, "utf8", "replace") \
                        if isinstance(product.name, str) \
                        else unicodedata.normalize('NFD', product.name)
                    quantity = str(product.quantity)
                    price = str(product.price_unit)
                    amount = product.quantity * product.price_unit
                    amount = str(round(
                        amount, decimal_precision_obj.precision_get(
                            cr, uid, 'Account')
                    ))
                    fields_one_product_invoice = [
                        item_number, quantity, price, amount
                    ]
                    one_product_invoice = ','.join([
                        '"%s"' % field_one_product_invoice
                        for field_one_product_invoice
                        in fields_one_product_invoice
                    ])
                    ...
                    if product.invoice_line_tax_id:
                        for one_tax in product.invoice_line_tax_id:
                            # or one_tax.description or one_tax.name
                            tax_name = one_tax.description
                            # 1=yes, 0=No
                            tax_included = str(1) if one_tax.price_include \
                                else str(0)
                            tax_refundable = str(1)  # 1=yes, 0=No
                            tax_rate = str(one_tax.amount)
                            tax_amount = str(one_tax.amount)
                            fields_tax_product_line = [
                                tax_name, tax_included, tax_refundable,
                                tax_rate, tax_amount,
                            ]
                            tax_product_line = ',' + ','.join([
                                '"%s"' % one_tax_field
                                for one_tax_field in fields_tax_product_line])
                        tax_product_line = tax_product_line.encode('UTF-8')
                    product_line_invoice_with_tax += one_product_invoice \
                        + tax_product_line + '\n'
        ...
        self.write(cr, uid, ids, {
            'state': 'get', 'data': out, 'name': this.name,
            'format': this.format},
            context=context)
